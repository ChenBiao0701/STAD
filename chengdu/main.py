...
embedding_dim = 256
map_size = (167,154)
input_size = 167*154+1
num_classes = 3
output_size = 167*154+3
token_dim = 128
hidden_size = 256
z_dim = 128
num_layers = 4
...
class Decoder(nn.Module):
    def __init__(self, output_dim, hidden_dim, embed_dim, num_layers, num_heads, num_classes):
        ...
        self.embedding = nn.Embedding(output_dim, self.embed_dim, padding_idx=0)
        self.pos_encoder = PositionalEncoding(self.embed_dim)
        decoder_layers = nn.TransformerDecoderLayer(d_model=self.embed_dim, nhead=num_heads)
        self.transformer_decoder = nn.TransformerDecoder(decoder_layers, num_layers=num_layers)
        self.fc_hidden = nn.Linear(hidden_dim, self.embed_dim)
        self.fc_memory = nn.Linear(hidden_dim, self.embed_dim)
        self.fc_label = nn.Linear(num_classes, self.embed_dim)
        self.out = nn.Linear(self.embed_dim, output_dim)
        
    def forward(self, input, encode_hidden, decode_hidden, lengths, y_oh):
        embedded = self.embedding(input)
        embedded = self.pos_encoder(embedded.transpose(0, 1))
        
        tgt_key_padding_mask = (input == 0).to(input.device)
        
        # Process encode_hidden
        memory = encode_hidden.transpose(0, 1)
        memory = self.fc_memory(memory)
        
        # Process labels
        y_oh = y_oh.float()  # Convert y_oh to float
        label_embed = self.fc_label(y_oh).unsqueeze(1)
        memory = memory + label_embed
        
        # Process decode_hidden
        decode_hidden = self.fc_hidden(decode_hidden.squeeze(0)).unsqueeze(0)
        
        output = self.transformer_decoder(embedded, memory.transpose(0, 1), tgt_key_padding_mask=tgt_key_padding_mask)
        prediction = self.out(output.transpose(0, 1))
        return prediction
...
def get_threshold(model, data):
    ...
    threshold = values[int(np.floor(len(values)*0.05))]
...
if __name__ == "__main__":
    ...
    decoder = Decoder(output_size, hidden_size, token_dim, num_layers, num_heads, num_classes)
    model = Seq2Seq(encoder, decoder, device, t_mu_shift=1.0, t_var_scale=1.0).to(device)
    criterion = nn.CrossEntropyLoss(reduction='none')
    optimizer = optim.SGD(model.parameters(), lr = lr, momentum=0.9)
    data = DataGenerator()
    data.load_outliers('train_cd_outliers.pkl', 'train')
    ...
    }, 'model_checkpoint_test1.pth')
    
    # 加载模型
    checkpoint = torch.load('model_checkpoint_test1.pth')
    model.load_state_dict(checkpoint['model_state_dict'])
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    threshold = get_threshold(model, data)

    data.load_outliers('test_cd_outliers.pkl', 'test')
