...
class DataGenerator:
    ...
    def build_dataset(self, data_type):
        data_name = "./data/processed_cd.csv".split('.')
    ...
    def inject_outliers(self, data_type, ratio=0.05, level=5, point_prob=0.3, vary=False):
        if data_type == "train":
            out_filename = 'train_cd_outliers.pkl'
            traj_num = self.train_traj_num
            trajectories = self.train_trajectories
        elif data_type == "test":
            out_filename = 'test_cd_outliers.pkl'
    ...
    def switching_batch(self, batch_x, level, prob, vary=False):
        ...
        for traj in batch_x:
            ...
            noisy_batch_x.append(traj[:anomaly_st_loc] +
                                 [self._detour_point(p, level, offset) for p in traj[anomaly_st_loc:anomaly_ed_loc]] +
                                 traj[anomaly_ed_loc:])
        return noisy_batch_x
...
data = DataGenerator()
data.inject_outliers(data_type='train', ratio=0.1, level=3, point_prob=0.5, vary=False)
data.inject_outliers(data_type='test', ratio=0.1, level=0, point_prob=0.5, vary=False)